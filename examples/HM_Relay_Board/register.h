...
EE::s_cnlTbl cnlTbl[] = {
  // cnl, lst, sIdx, sLen, pAddr;
    ...
    {  2,  1, 0x04,  4,  0x00ac, 0, },
    {  2,  3, 0x08, 22,  0x00b0, 0, },
    {  3,  1, 0x04,  4,  0x0134, 0, },
    {  3,  3, 0x08, 22,  0x0138, 0, },
    {  4,  1, 0x04,  4,  0x01bc, 0, },
    {  4,  3, 0x08, 22,  0x01c0, 0, },
}; 
...
*/  
	...
	hm.bt.set(0,0); // disable                                          // set battery check, internal, 2.7 reference, measurement each hour
	...
	else if( devIdnt[2] == 0x0a ) {
	  devDef.cnlNbr = 2;
	}
	devDef.lstNbr = 1 + (devDef.cnlNbr * 2);

    // register user modules
	for( uint8_t i=0; i<devDef.cnlNbr; ++i ) {
	  cmSwitch[i].regInHM(i+1, 3, &hm);                                    // register user module
	  cmSwitch[i].config(&initRly, &switchRly);                          // configure user module
	}
}
